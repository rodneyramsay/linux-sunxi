...
static unsigned nand_gpios_handle = -1;
...
//config dma
	...
	{
		/* config para */
		memset(&nand_dma_config, 0, sizeof(nand_dma_config));
		nand_dma_config.xfer_type.src_data_width 	= DATA_WIDTH_32BIT;
		nand_dma_config.xfer_type.src_bst_len 	= DATA_BRST_4;
		nand_dma_config.xfer_type.dst_data_width 	= DATA_WIDTH_32BIT;
		nand_dma_config.xfer_type.dst_bst_len 	= DATA_BRST_4;
		nand_dma_config.address_type.src_addr_mode 	= DDMA_ADDR_IO; 
		nand_dma_config.address_type.dst_addr_mode 	= DDMA_ADDR_LINEAR;
		nand_dma_config.src_drq_type 	= D_DST_NAND;
		nand_dma_config.dst_drq_type 	= D_DST_SDRAM;
		nand_dma_config.bconti_mode 		= false;
		nand_dma_config.irq_spt 		= CHAN_IRQ_FD;
//        printk("nand read config done!");
	...
	else //write to nand
	{
		...
		nand_dma_config.xfer_type.dst_data_width 	= DATA_WIDTH_32BIT;
		nand_dma_config.xfer_type.dst_bst_len 	= DATA_BRST_4;
		nand_dma_config.address_type.src_addr_mode 	= DDMA_ADDR_LINEAR;
		nand_dma_config.address_type.dst_addr_mode 	= DDMA_ADDR_IO;
		nand_dma_config.src_drq_type 	= D_DST_SDRAM;
		nand_dma_config.dst_drq_type 	= D_DST_NAND;
		nand_dma_config.bconti_mode 		= false;
		nand_dma_config.irq_spt 		= CHAN_IRQ_FD;
//        printk("nand write config done!");
	...
	if(0 != sw_dma_config(dma_hdle, &nand_dma_config)) {
...
{
	nand_gpios_handle = gpio_request_ex("nand_para", NULL);

	if (0 == nand_gpios_handle) {
		pr_err("nandlib: request nand gpio set failed\n");
		return;
	}
}
#else
...
{
	gpio_release(nand_gpios_handle, 2);
}
...
int NAND_get_storagetype(void)
{
	...
	pr_debug("[%s] enter\n", __FUNCTION__);
	err = script_parser_fetch("target", "storage_type", &storage_type,
				  sizeof(storage_type)/sizeof(int));

	if (err) {
		pr_err("[%s] get storage_type err\n", __FUNCTION__);
		storage_type = 0;
	}
