...
static int  init_blklayer(void)
{
	...
	err = script_parser_fetch("nand_para", "good_block_ratio", &nand_good_block_ratio,
				  sizeof(nand_good_block_ratio)/sizeof(int));

	if (err) {
		pr_err("[%s] get good_block_ratio err\n", __FUNCTION__);
...
int nand_init(void)
{
	...
	err = script_parser_fetch("nand_para", "nand_used", &nand_used,
				  sizeof(nand_used)/sizeof(int));

	if (err) {
		pr_err("[%s] get nand_used err\n", __FUNCTION__);
		nand_used = 0;
	}
